  ...
  if (it.createErrors !== false) {
    ...
    if (typeof $schema == 'object') {
      ...
      if ($keywordErrorsArr.length) {
        ...
        out += ' ***REMOVED*** while (' + ($i) + ' < errors) { ' + ($err) + ' = vErrors[' + ($i) + ']; if (  ' + ($err) + '.keyword != \'' + ($keyword) + '\' ';
        ...
        out += ' ***REMOVED***  if (vErrors === null) vErrors = [err]; else vErrors.push(err); errors++; ';
        if (!$config.options.singleError) {
          out += ' } } ';
        }
      }
      ...
      if ($hasProperties || $hasItems) {
        ...
        if ($hasProperties && $hasItems) {
          ...
          out += ' ***REMOVED*** } else { var ' + ($childKeyword) + ' = \'properties\'; ' + ($errors) + ' =  ' + (JSON.stringify($childErrors.properties)) + ';    ';
