...
{{? it.createErrors !== false }}
  ...
  if (errors > 0) {
    var {{=$dataPath}} = (dataPath || '') + {{= it.errorPath }***REMOVED***
    var {{=$i}}, {{=$err}}, {{=$errors}***REMOVED***
    ...
    {{? typeof $schema == 'object' }}
      ...
      {{? $keywordErrorsArr.length }}
        {{=$i}} = 0;
        {{=$errors}} = {{= JSON.stringify($keywordErrors) }***REMOVED***
        ...
        {{? $config.options.singleError }}
              ...
              {{=$message}} += {{# def.em_errorMessage }***REMOVED***
              {{=$paramsErrors}} = {{=$paramsErrors}}.concat({{=$errors}}[{{=$key}}]);
            }
          }
        {{??}}
              var {{=$message}} = {{# def.em_errorMessage }***REMOVED***
      ...
      {{? $hasKeywordProps }}
        {{=$i}} = 0;
        {{=$errors}} = {{= JSON.stringify($keywordPropErrors) }***REMOVED***
        var {{=$paramsErrors}}, {{=$propParam}***REMOVED***
        var {{=$keywordPropParams}} = {{= JSON.stringify($config.KEYWORD_PROPERTY_PARAMS) }***REMOVED***
          ...
          {{? $hasProperties && $hasItems }}
            (typeof {{=$data}} == 'object') {
              ...
              if ({{=$isArray}}) {
                ...
                {{=$errors}} = {{= JSON.stringify($childErrors.items) }***REMOVED*** 
                {{# def.em_compileChildTemplates: items }}
              } else {
                var {{=$childKeyword}} = 'properties';
                {{=$errors}} =  {{= JSON.stringify($childErrors.properties) }***REMOVED*** 
