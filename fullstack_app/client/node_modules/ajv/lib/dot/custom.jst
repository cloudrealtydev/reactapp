...
{{## def.extendErrors:_inline:
  for (var {{=$i}}={{=$errs}***REMOVED*** {{=$i}}<errors; {{=$i}}++) {
    var {{=$ruleErr}} = vErrors[{{=$i}}];
    if ({{=$ruleErr}}.dataPath === undefined)
      {{=$ruleErr}}.dataPath = (dataPath || '') + {{= it.errorPath }***REMOVED***
    {{# _inline ? 'if (\{\{=$ruleErr\}\}.schemaPath === undefined) {' : '' }}
      {{=$ruleErr}}.schemaPath = "{{=$errSchemaPath}}";
    {{# _inline ? '}' : '' }}
    {{? it.opts.verbose }}
      {{=$ruleErr}}.schema = {{=$schemaValue}***REMOVED***
      {{=$ruleErr}}.data = {{=$data}***REMOVED***
    {{?}}
  }
#}}
...
{{? $inline }}
  ...
  {{??}}
    {{=$valid}} = {{= $ruleValidate.validate }***REMOVED***
  {{?}}
{{?? $macro }}
  {{# def.setupNextLevel }}
  {{
...
{{??}}
  ...
  {{? $rDef.errors === false }}
    {{=$valid}} = {{? $asyncKeyword }}await {{?}}{{= def_callRuleValidate }***REMOVED***
  {{??}}
    {{? $asyncKeyword }}
      {{ $ruleErrs = 'customErrors' + $lvl; }}
      var {{=$ruleErrs}} = null;
      try {
        {{=$valid}} = await {{= def_callRuleValidate }***REMOVED***
      } catch (e) {
        {{=$valid}} = false;
    ...
    {{??}}
      {{=$ruleErrs}} = null;
      {{=$valid}} = {{= def_callRuleValidate }***REMOVED***
    {{?}}
  {{?}}
{{?}}
    ...
    {{??}}
      ...
      {{??}}
        if (Array.isArray({{=$ruleErrs}})) {
          if (vErrors === null) vErrors = {{=$ruleErrs}***REMOVED***
