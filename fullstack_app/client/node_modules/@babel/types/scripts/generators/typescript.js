...
export type Node = ${t.TYPES.sort().join(" | ")***REMOVED***\n\n`;
...
for (const type in t.NODE_FIELDS) {
  ...
  fieldNames.forEach(fieldName => {
    ...
    if (t.isValidIdentifier(fieldName) || alphaNumeric.test(fieldName)) {
      struct.push(`${fieldName}: ${typeAnnotation***REMOVED***`);
    } else {
      struct.push(`"${fieldName}": ${typeAnnotation***REMOVED***`);
    }
  });
  ...
  if (type !== "Super" && type !== "Import") {
    lines.push(
      `export function ${toFunctionName(type)}(${args.join(", ")}): ${type***REMOVED***`
    );
  } else {
    const functionName = toFunctionName(type);
    lines.push(
      `declare function _${functionName}(${args.join(", ")}): ${type***REMOVED***`,
...
for (let i = 0; i < t.TYPES.length; i++) {
  ...
  if (t.NODE_FIELDS[t.TYPES[i]]) {
    decl += `node is ${t.TYPES[i]***REMOVED***`;
 ...
 */
export type ${type} = ${t.DEPRECATED_KEYS[type]***REMOVED***\n
`;
}
