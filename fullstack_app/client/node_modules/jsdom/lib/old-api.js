...
exports.jsdom = function (html, options) {
  ...
  if (options.parsingMode !== "html" && options.parsingMode !== "xml") {
    throw new RangeError(`Invalid parsingMode option ${JSON.stringify(options.parsingMode)***REMOVED*** must be either "html", ` +
...
function setGlobalDefaultConfig(config) {
  config.parseOptions = { locationInfo: true ***REMOVED***

  config.pool = config.pool !== undefined ? config.pool : { maxSockets: 6 ***REMOVED***

  config.agentOptions = config.agentOptions !== undefined ?
                        config.agentOptions :
                        { keepAlive: true, keepAliveMsecs: 115 * 1000 ***REMOVED***

  config.strictSSL = config.strictSSL !== undefined ? config.strictSSL : true;

  config.userAgent = config.userAgent ||
    `Node.js (${process.platform***REMOVED*** U; rv:${process.version}) AppleWebKit/537.36 (KHTML, like Gecko)`;
}
