...
function Document(obj, fields, skipId, options) {
  ...
  this.$__.$options = options || {***REMOVED***
...
function $__hasIncludedChildren(fields) {
  const hasIncludedChildren = {***REMOVED***
  ...
  function _init(index) {
    ...
    if (!schema && utils.isPOJO(obj[i])) {
      ...
      if (obj[i] === null) {
        ...
        const intCache = obj[i].$__ || {***REMOVED***
...
Document.prototype.$set = function $set(path, val, type, options) {
  ...
  try {
    ...
    if (schema.options &&
      ...
      if (this.ownerDocument) {
        popOpts = { [populateModelSymbol]: val[0].constructor ***REMOVED***
        this.ownerDocument().populated(this.$__fullPath(path),
  ...
  for (; i < l; i++) {
    ...
    if (last) {
      ...
      if (utils.isPOJO(obj[parts[i]])) {
        ...
        obj[parts[i]] = obj[parts[i]] || {***REMOVED***
        obj = obj[parts[i]];
      }
    }
  }
 ...
 *
 * @param {Object} [options]
 * @param {Boolean} [options.includeChildren=false] if true, returns children of modified paths as well. For example, if false, the list of modified paths for `doc.colors = { primary: 'blue' ***REMOVED***` will **not** contain `colors.primary`. If true, `modifiedPaths()` will return an array that contains `colors.primary`.
 * @return {Array}
 * @api public
 */
...
function _getPathsToValidate(doc) {
  let i;
  let len;
  const skipSchemaValidators = {***REMOVED***
  ...
  const flattenOptions = { skipArrays: true ***REMOVED***
  len = paths.length;
  for (i = 0; i < len; ++i) {
...
Document.prototype.$__validate = function(options, callback) {
  ...
  const _complete = () => {
    ...
    this.$__.cachedRequired = {***REMOVED***
...
Document.prototype.$__reset = function reset() {
  ...
  this.$__.activePaths
    ...
    .forEach(function(array) {
      ...
      array[arrayAtomicsSymbol] = {***REMOVED***
    });
  ...
  this.$__dirty().forEach(function(dirt) {
    ...
    if (type && type[arrayAtomicsSymbol]) {
      type[arrayAtomicsSymbol] = {***REMOVED***
    }
  });
...
Document.prototype.$__dirty = function() {
  ...
  all.forEach(function(item) {
    ...
    if (lastPath == null || item.path.indexOf(lastPath) !== 0) {
      ...
      // of entire array.
      top.value[arrayAtomicsSymbol] = {***REMOVED***
...
Document.prototype.$toObject = function(options, json) {
  ...
  ***REMOVED***
  ...
  options = utils.isPOJO(options) ? clone(options) : {***REMOVED***
  ...
  let ret = clone(this._doc, cloneOptions) || {***REMOVED***
...
function applyVirtuals(self, json, options, toObjectOptions) {
  ...
  for (i = 0; i < numPaths; ++i) {
    ...
    for (let j = 0; j < plen - 1; ++j) {
      cur[parts[j]] = cur[parts[j]] || {***REMOVED***
      cur = cur[parts[j]];
    }
    cur[parts[plen - 1]] = v;
  }
    ...
    if (this.$session() != null) {
      ...
      paths.forEach(path => {
        if (path.options == null) {
          path.options = { session: session ***REMOVED***
          return;
        }
