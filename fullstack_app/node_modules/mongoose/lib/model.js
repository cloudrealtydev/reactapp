...
Model.discriminator = function(name, schema, value) {
  ...
  if (this[subclassedSymbol] != null) {
    for (const submodel of this[subclassedSymbol]) {
      submodel.discriminators = submodel.discriminators || {***REMOVED***
...
Model.create = function create(doc, options, callback) {
  ...
  if (Array.isArray(doc)) {
    ...
    options = options != null && typeof options === 'object' ? options : {***REMOVED***
  } else {
...
Model.mapReduce = function mapReduce(o, callback) {
  ...
  return utils.promiseOrCallback(callback, cb => {
    if (!Model.mapReduce.schema) {
      const opts = {noId: true, noVirtualId: true, strict: false***REMOVED***
 ...
 * ####Example:
 *
 *     var options = { near: [10, 10], maxDistance: 5 ***REMOVED***
  ...
  for (let i = 0; i < len; ++i) {
    ...
    if (mod.foreignField.size === 1) {
      ...
      if (foreignField !== '_id' || !match['_id']) {
        ...
        match[foreignField] = { $in: ids ***REMOVED***
      }
    } else {
      const $or = [];
...
function _assign(model, vals, mod, assignmentOpts) {
  ...
  const rawOrder = {***REMOVED***
  const rawDocs = {***REMOVED***
  let key;
  let val;
...
function _formatMatch(match) {
  if (Array.isArray(match)) {
    if (match.length > 1) {
      return { $or: [].concat(match.map(m => Object.assign({}, m))) ***REMOVED***
    }
...
Model.compile = function compile(name, schema, collectionName, connection, base) {
  ...
  ***REMOVED***
  ...
  const _userProvidedOptions = schema._userProvidedOptions || {***REMOVED***
...
Model.__subclass = function subclass(conn, schema, collection) {
  ...
  ***REMOVED***
  ...
  const options = s.options || {***REMOVED***
  const _userProvidedOptions = s._userProvidedOptions || {***REMOVED***
