...
class ClientSession extends EventEmitter {
  ...
  constructor(topology, sessionPool, options, clientOptions) {
    ...
    clientOptions = clientOptions || {***REMOVED***
...
function endTransaction(session, commandName, callback) {
  ...
  const command = { [commandName]: 1 ***REMOVED***
  ...
  if (session.transaction.options.writeConcern) {
    ...
    writeConcern = { w: session.clientOptions.w ***REMOVED***
  }
...
class ServerSession {
  constructor() {
    this.id = { id: new Binary(uuidV4(), Binary.SUBTYPE_UUID) ***REMOVED***
...
function applySession(session, command, options) {
  ...
  if (!inTransaction) {
    ...
    if (session.supports.causalConsistency && session.operationTime && shouldApplyReadConcern) {
      command.readConcern = command.readConcern || {***REMOVED***
